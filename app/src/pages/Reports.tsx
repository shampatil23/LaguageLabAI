        ...
        <div className="flex gap-2 flex-wrap self-start sm:self-auto">
          <button
            onClick={() => {
              ...
              // Build a real .xlsx workbook with proper column widths
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-white text-indigo-700 text-sm font-bold rounded-xl hover:bg-indigo-50 shadow-md transition-all hover:scale-[1.03]"
          >
          ...
          <button
            onClick={() => {
              ...
              doc.text('Test Reports & Analytics', 14, 16);
              doc.setFontSize(10);
              doc.setFont('helvetica', 'normal');
              doc.setTextColor(120);
              doc.text(`Generated on ${new Date().toLocaleDateString()}  |  ${filtered.length} results  |  Average score: ${avgScore}%`, 14, 23);
              autoTable(doc, {
                startY: 28,
                head: [['Student', 'Lesson', 'Course', 'Session', 'Score', 'Correct', 'Date']],
                body: filtered.map(r => [
                  r.studentName || '',
                  r.lessonName || '',
                  r.courseTitle || '',
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-white/20 text-white border border-white/40 text-sm font-bold rounded-xl hover:bg-white/30 shadow-md transition-all hover:scale-[1.03]"
          >
