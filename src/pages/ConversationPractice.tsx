...
import { Mic, MicOff, AlertCircle, Bot, User, Send, Loader2, FileText, CheckCircle, XCircle, AlertTriangle, Plus, X, ChevronRight, BarChart2, BookOpen, TrendingUp, History, Clock, Star, PenLine, Download } from 'lucide-react';
...
Base the analysis on the student's actual messages. If no grammar errors exist, return empty array for grammarCorrections. Return ONLY the JSON object, nothing else.`;
  ...
  const handleExportReportPDF = () => {
    ...
    // Build a printable HTML page for the report and open print dialog
    const html = `
<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8">
  <title>AI Session Report - ${report.topic}</title>
  <style>
    ...
    .score-card { background: linear-gradient(135deg, #4f46e5, #1d4ed8); color: white; padding: 20px; border-radius: 12px; margin-bottom: 16px; }
    .score-num { font-size: 48px; font-weight: 900; }
    .score-summary { font-size: 12px; opacity: 0.85; margin-top: 8px; }
    .section { margin: 16px 0; }
    .section h3 { font-size: 14px; font-weight: 700; margin-bottom: 8px; padding: 6px 10px; border-radius: 6px; }
    .strengths h3 { background: #d1fae5; color: #065f46; }
    .weaknesses h3 { background: #fee2e2; color: #991b1b; }
    .tips h3 { background: #dbeafe; color: #1e40af; }
    .grammar h3 { background: #fef3c7; color: #92400e; }
    ul { margin: 0; padding-left: 20px; }
    li { font-size: 12px; margin: 4px 0; line-height: 1.5; }
    .grammar-item { border: 1px solid #fcd34d; border-radius: 8px; padding: 10px; margin: 8px 0; }
    .wrong { text-decoration: line-through; color: #dc2626; font-size: 12px; }
    .right { color: #059669; font-weight: 700; font-size: 12px; }
    .explain { color: #92400e; font-size: 11px; margin-top: 4px; }
    .footer { text-align: center; font-size: 10px; color: #94a3b8; margin-top: 24px; border-top: 1px solid #e2e8f0; padding-top: 12px; }
    @media print { body { margin: 0; } }
  </style>
</head>
<body>
  <h1>AI Conversation Session Report</h1>
  <p class="subtitle">Topic: ${report.topic} &nbsp;|&nbsp; Date: ${new Date().toLocaleDateString()} &nbsp;|&nbsp; Score: ${report.overallScore}/100</p>

  <div class="score-card">
    <div class="score-num">${report.overallScore}<span style="font-size:20px;opacity:0.7">/100</span></div>
  ...
  <div class="section tips">
    ...
    <ul>${report.improvements.map((t, i) => `<li>${i + 1}. ${t}</li>`).join('')}</ul>
  </div>

  ${report.grammarCorrections && report.grammarCorrections.length > 0 ? `
  <div class="section grammar">
    <h3>📝 Grammar Corrections</h3>
    ${report.grammarCorrections.map(gc => `
    <div class="grammar-item">
      <div class="wrong">✗ ${gc.original}</div>
      <div class="right">✓ ${gc.corrected}</div>
      <div class="explain">${gc.explanation}</div>
    </div>`).join('')}
  </div>` : '<p style="color:#059669;font-size:12px;">✓ No significant grammar errors detected!</p>'}

  <div class="footer">Generated by LinguaLab AI &bull; ${new Date().toLocaleString()}</div>
</body>
</html>`;
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            {report && !isGeneratingReport && (
              <Button onClick={handleExportReportPDF} variant="outline" className="text-slate-700 border-slate-200 hover:bg-slate-50 text-sm">
